...
class TESNet(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f'prototype_distances: {time.perf_counter() - start_time}')

        start_time = time.perf_counter()
        cosine_min_distances = self.global_min_pooling(cosine_distances)
        torch.cuda.synchronize()
        print(f'cosine_min_distances: {time.perf_counter() - start_time}')
        ...
        torch.cuda.synchronize()
        print(f'global_max_pooling, logits: {time.perf_counter() - start_time}')
