...
top5_probs, top5_indices = torch.topk(logits[idx], 5)

log('Top-5 Predicted Classes:')
for rank, (class_idx, prob) in enumerate(zip(top5_indices.tolist(), top5_probs.tolist()), start=1):
    class_identity = prototype_img_identity[class_idx] if class_idx < len(prototype_img_identity) else "Unknown"
    log(f"Rank {rank}: Class {class_idx} (Identity: {class_identity}), Confidence: {prob:.4f}")

# Retrieve and log class identities (assuming `class_id_to_name` maps class indices to class names)
predicted_class_identity = prototype_img_identity[predicted_cls] if predicted_cls in prototype_img_identity else "Unknown"
actual_class_identity = prototype_img_identity[correct_cls] if correct_cls in prototype_img_identity else "Unknown"

log('Predicted Class Identity: ' + str(predicted_class_identity))
log('Actual Class Identity: ' + str(actual_class_identity))
